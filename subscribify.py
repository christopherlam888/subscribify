...
def getVideos(channel, n):
    ...
    for video in data["latestVideos"][:n]:
        channel_videos.append(
            Video(video["title"], data["author"], video["published"], video["videoId"])
        )
...
def main():
    ...
    with multiprocessing.Pool() as pool:
        ...
        for channel_videos in tqdm.tqdm(
            pool.istarmap(getVideos, params), total=len(channels)
        ):
    ...
    while True:
        try:
            ...
            if 0 < selection <= len(videos):
                print(f"URL:\n{url_start+videos[selection-1].videoId}")
                webbrowser.open(url_start + videos[selection - 1].videoId, new=2)
        except ValueError:
            break
